...
def infer_sample_name(fq1):
    name = Path(fq1).name
    pats = [r"_R1\.f(ast)?q\.gz$", r"_1\.f(ast)?q\.gz$", r"\.R1\.f(ast)?q\.gz$", r"-R1\.f(ast)?q\.gz$", r"\.1\.f(ast)?q\.gz$"]
    for pat in pats:
        if re.search(pat, name): return re.sub(pat, "", name)
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="Align, call variants, split SNVs/INDELs, emit MultiQC custom content.")
    ap.add_argument("fq1")
    ap.add_argument("fq2")
    ap.add_argument("ref")
    ap.add_argument("--ploidy", default="2", help="Ploidy for bcftools call (default: 2)")
    ap.add_argument("--threads", type=int, default=4, help="CPUs for bwa/samtools/bcftools")
    ap.add_argument("--outdir", default="results", help="Output directory (default: results)")
    args = ap.parse_args()

    fq1, fq2, ref = args.fq1, args.fq2, args.ref
    if not Path(fq1).exists() or not Path(fq2).exists(): sys.exit("Input FASTQs not found.")
    if not Path(ref).exists(): sys.exit("Reference FASTA not found.")

    sample = infer_sample_name(fq1)
    results_dir = Path(os.getcwd()) / args.outdir
    ...
    # 2) Index BAM (retry without -@ if not supported)
    ...
    # 3) mpileup -> 4) call  (bcftools uses --threads)
    cmd_call = (
        f"bcftools mpileup --threads {args.threads} -Ou -f '{ref}' "
        f"-a FORMAT/DP -q 10 -Q 13 '{sorted_bam}' "
        f"| bcftools call --threads {args.threads} -mv --ploidy {args.ploidy} -Oz -o '{final_vcf_gz}'"
    )
    print(f"[run] {cmd_call}")
    run_bash(cmd_call)

    # 5) Index VCF (no threads flag)
    ...
    # 6) Split SNVs / INDELs (view supports --threads)
    print(f"[run] bcftools view --threads {args.threads} -v snps -Oz -o {snv_gz} {final_vcf_gz}")
    run(["bcftools", "view", "--threads", str(args.threads), "-v", "snps", "-Oz", "-o", str(snv_gz), str(final_vcf_gz)])
    ...
    print(f"[run] bcftools view --threads {args.threads} -v indels -Oz -o {indel_gz} {final_vcf_gz}")
    run(["bcftools", "view", "--threads", str(args.threads), "-v", "indels", "-Oz", "-o", str(indel_gz), str(final_vcf_gz)])
